...
# 힙은 최소 값순으로 정렬되므로 최대 값순으로 정렬하고 싶기 때문에 배열의 원소 값을 -로 적용한다.
# 해당 배열을 힙으로 적용하고
# 제일 앞의 최대 값을 꺼내오고 
# -최대값이므로 -1이 아닌 +1로 차감해준 값으로 다시 넣어 최신화해준다.
# 어차피 제곱으로 합산하기에 -로 적용한 것을 그대로 두어도 문제없다.
...
def solution(n, works):
    ...
    works = [-i for i in works]                 # -로 한 적용한 이유는 힙은 최소 값순으로 정렬되므로 최대 값순으로 정렬하고 싶기 때문이다. 
    ...
    for i in works:
        answer += i ** 2                        # 어차피 제곱으로 합산하기에 -로 적용한 것을 그대로 두어도 문제 없는 것
